...
const Chat = () => {
    ...
    return (
        <div className=' bg-gray-100 h-11 '>
            {/* toggle button for side chats */}
            <button onClick={handleClick} data-drawer-target="default-sidebar" data-drawer-toggle="default-sidebar"
                aria-controls="default-sidebar" type="button"
                className="  border-r-2 border-gray-200 bg-gray-100 pl-3 w-12  inline-flex items-center py-2  text-sm text-gray-500 hover:bg-gray-200 ">
            ...
            <aside id="default-sidebar" className={`border-r-2 border-gray-200 fixed bg-gray-100 left-0 z-40 w-12 h-screen transition-transform ${show}  `} aria-label="Sidebar">
                <div className="h-64 px-3 pt-2 overflow-y-auto  ">
                    <ul className="space-y-2 font-medium">
...

                        {
                            arr.map((item) => (
                                <li key={item.path}>
                                    {item.not && <Notification path={item.path} col={item.col} fill={item.fill} />}


                                    {!item.not && <button className="bg-white rounded-full ml-1 ">
                                        <svg width="22" height="22" viewBox="0 0 30 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <g id="Icon">
                                                <path id="Icon_2" fill-rule="evenodd" clip-rule="evenodd" d={item.path} fill={item.fill} />
                                            </g>
                                        </svg>
                                    </button>}
                ...
                <div className=" px-3 pt-16 overflow-y-auto bg-gray-100 rounded-b-full">
                    <ul className="space-y-2 font-medium">

                        {profiles.map(profile => (
                            <li key={profile.n} className=' flex'>

                                <button className={`${profile.col} rounded-full`}>
                                    <Image
